...
const formSchema = z.object({
  paymentType: z.enum(['sale', 'purchase'], { required_error: "Debe seleccionar un tipo." }),
  entityId: z.string().min(1, { message: "Por favor seleccione una entidad." }),
  selectedInvoiceIds: z.record(z.boolean()),
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent>
                           <SelectItem value="sale">Pago de Cliente (Venta)</SelectItem>
                           <SelectItem value="purchase">Pago a Proveedor (Compra)</SelectItem>
                        </SelectContent>
                      </Select>
                    <FormMessage />
                  </FormItem>
                    ...
                    <TableHeader>
                      <TableRow>
                        <TableHead className="w-10">
                          <Checkbox
                             checked={Object.values(selectedInvoiceIds).every(Boolean) && Object.keys(selectedInvoiceIds).length === invoicesWithBalance.length}
                      ...
                      {invoicesWithBalance.map((invoice) => (
                        <TableRow key={invoice.id}>
                          <TableCell>
                            <FormField
                                control={form.control}
                                name={`selectedInvoiceIds.${invoice.id}`}
                                render={({ field }) => (
                                <Checkbox
                                    checked={field.value}
          ...
          <div className="flex justify-end gap-2 pt-4">
              ...
              <Button type="submit" disabled={isSubmitting || !paymentPreview}>
                  {isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                  {isSubmitting ? 'Guardando...' : 'Registrar Pago Masivo'}
              </Button>
          </div>
        </form>
      </Form>
